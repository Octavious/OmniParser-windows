...
if sys.platform == 'win32':
    ...
    # Disable progress bar for Windows
    os.environ['EASYOCR_DISABLE_PROGRESS_BAR'] = '1'
...
def get_som_labeled_img(image_source: Union[str, Image.Image], model=None, BOX_TRESHOLD=0.01, output_coord_in_ratio=False, ocr_bbox=None, text_scale=0.4, text_padding=5, draw_bbox_config=None, caption_model_processor=None, ocr_text=[], use_local_semantics=True, iou_threshold=0.9,prompt=None, scale_img=False, imgsz=None, batch_size=128):
    ...
    try:
        ...
        if not xyxy_elem:
            print("No valid YOLO boxes detected")
            return None, None, None
            
        filtered_boxes = remove_overlap_new(boxes=xyxy_elem, iou_threshold=iou_threshold, ocr_bbox=ocr_bbox_elem)
        
        # Sort boxes and get starting index
        filtered_boxes_elem = sorted(filtered_boxes, key=lambda x: x['content'] is None)
        starting_idx = next((i for i, box in enumerate(filtered_boxes_elem) if box['content'] is None), -1)
        filtered_boxes = torch.tensor([box['bbox'] for box in filtered_boxes_elem])
        
        print(f'Processing {len(filtered_boxes)} boxes, starting from index {starting_idx}')
        ...
        if use_local_semantics and caption_model_processor:
            ...
            # Fill in content for boxes
            for i, box in enumerate(filtered_boxes_elem):
                if box['content'] is None and i - starting_idx < len(parsed_content_icon):
                    box['content'] = parsed_content_icon[i - starting_idx]
...
def check_ocr_box(image_source: Union[str, Image.Image], display_img = True, output_bb_format='xywh', goal_filtering=None, easyocr_args=None, use_paddleocr=False):
    try:
        ...
        print(f"Error in check_ocr_box: {str(e)}")
        return ([], []), goal_filtering